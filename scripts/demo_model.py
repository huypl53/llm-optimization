...
def build_conversation(image_pil: Image.Image, prompt: str, system_prompt: str) -> list:
    return [
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run vLLM chat on each image in a local directory."
    )
    ...
    with args.output.open("w", encoding="utf-8", newline="") as f:
        ...
        for image_path in image_paths:
            ...
            suffix = f" [resample {occurrence_idx + 1}]" if occurrence_idx > 0 else ""
            print(
                f"{image_path.name}{suffix}: {generated_text} (latency {latency:.4f}s)"
            )

        avg_latency = sum(latencies) / len(latencies)
        writer.writerow(["SUMMARY", f"tested={len(latencies)}", f"{avg_latency:.4f}"])
